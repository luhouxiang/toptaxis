...
static char log_buffer[LOG_MAX_BUF_LENGTH] = {0};
...
static inline void log_file_name(int lvl, int seq, char* file_name, time_t now)
{
	...
	localtime_r(&now, &tm);
	snprintf (file_name, FILENAME_MAX-1, "%s/%s%s%04d%02d%02d%03d", log_dir, log_pre, 
					((char*[]){
						"emerg", "alert", "crit",
						"error", "warn", "notice",
						"info", "debug", "trace"
...
//	struct stat buf;
	...
	/*Modify by  begin */
	/*20100312: The code below has no use, and the parameter seq is wrong, so i delete this part*/
	/*
	log_file_name (lvl, --seq, file_name, now);
...
#define BOOT_FAIL	"\e[1m\e[31m[ failed ]\e[m"
    ...
    #endif
	va_start(ap, fmt);
	end = vsnprintf (log_buffer, LOG_MAX_BUF_LENGTH-1, fmt, ap);
	va_end (ap);
	...
	strncat (log_buffer, OK == 0 ? BOOT_OK : BOOT_FAIL, LOG_MAX_BUF_LENGTH-1);
	printf ("\r%s\n", log_buffer);
...
void write_log (int lvl, const char* fmt, ...)
{
	...
	if (lvl == APP_INFO || lvl == APP_NOTICE || lvl == APP_WARNING)
		pos = snprintf (log_buffer, LOG_MAX_BUF_LENGTH-1, "[%02d:%02d:%02d]", 
			tm.tm_hour, tm.tm_min, tm.tm_sec);
	else
		pos = snprintf (log_buffer, LOG_MAX_BUF_LENGTH-1, "[%02d:%02d:%02d][%05d]", 
