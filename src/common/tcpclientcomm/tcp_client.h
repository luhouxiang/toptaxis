...
*
*           如果想在多线程中调用，请在线程内部定义该CTCPClientSocket类
*
*参考范例: 见test.cpp
****************************************************************/
...
class CTaxisTCPClientSocket
{
public:
    CTaxisTCPClientSocket();
    ...
    int SendAndRcvData(char* szReqBuf, unsigned int iReqLen, char* szRspBuf, unsigned int iRspMaxLen, char* szErrorMsg, unsigned int iMaxErrorLen, int& iRecvLen); 
...
private:
    ...
    int SendData(char* szReqBuf, unsigned int iReqLen,char* szErrorMsg, unsigned int iMaxErrorLen);
