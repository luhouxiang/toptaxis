...
************************************************************/
int CTCPClientSocket::InitServer(const char* szIP, unsigned short usPort, unsigned int iTimeOut, char* szErrorMsg, unsigned int iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
...
*函数功能: 发送请求，并接收响应
************************************************************/
int CTCPClientSocket::SendAndRcvData(char* szReqBuf, unsigned int iReqLen, char* szRspBuf, unsigned int iRspMaxLen, char* szErrorMsg, unsigned int iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
		return LEVEL_SYSTEM_ERROR;
	...
	if(iRspMaxLen < MAX_UNSIGNED_INT_LEN)
	{
		snprintf(szErrorMsg, iMaxErrorLen, "[SendAndRcvData] szRspBuf is too short! bufLen[%u]<[%u]", iRspMaxLen, MAX_UNSIGNED_INT_LEN);
		return LEVEL_RETURN_FAILED;
	}

	/*先接收4个字节的头部*/
	iRet = RecvData(szRspBuf, iRspMaxLen, MAX_UNSIGNED_INT_LEN, szErrorMsg, iMaxErrorLen);
	if(iRet != LEVEL_RETURN_SUCCESS)
	{
		return iRet;
	}
	...
	/*获取消息长度*/
	uiMsgLen = *((unsigned int *)szRspBuf);
	...
	/*在接收uiMsgLen 字节的消息体*/
	iRet = RecvData(szRspBuf+MAX_UNSIGNED_INT_LEN, iRspMaxLen, uiMsgLen - MAX_UNSIGNED_INT_LEN, szErrorMsg, iMaxErrorLen);
	if(iRet != LEVEL_RETURN_SUCCESS)
	{
		return iRet;
	}
...
int CTCPClientSocket::SendDataWithoutRecv(const char* szReqBuf, unsigned int iReqLen, char* szErrorMsg, unsigned int iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
		return LEVEL_SYSTEM_ERROR;
	...
	if(NULL == szReqBuf)
	{
		snprintf(szErrorMsg, iMaxErrorLen, "%s", "[SendDataWithoutRecv] Input parameter [szReqBuf] is NULL");
		return LEVEL_RETURN_FAILED;
	}
...
int CTCPClientSocket::IsConnected( fd_set *rdSet, fd_set *wrSet, fd_set *exSet )
{
	int iError = 0;
	...
	if (!FD_ISSET( m_iSock, rdSet ) && !FD_ISSET( m_iSock, wrSet ) )
	{
		return LEVEL_RETURN_FAILED;
	}
	*/
...
int CTCPClientSocket::CreateAndConnect(char* szErrorMsg, int iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
		return LEVEL_SYSTEM_ERROR;
	...
	//setsockopt(m_iSock,SOL_SOCKET,SO_DONTLINGER,(const char*)&bDontLinger,sizeof(bool));
...
int CTCPClientSocket::Reconnect(char* szErrorMsg, unsigned int iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
		return LEVEL_SYSTEM_ERROR;
...
int CTCPClientSocket::JudgeEnd(char* szRspBuf, unsigned int iMsgLen)
{
	if(NULL == szRspBuf)
	{
		return LEVEL_SYSTEM_ERROR;
...
int CTCPClientSocket::RecvData(char* szRspBuf, unsigned int iRspMaxLen, unsigned int iNeedLen, char*  szErrorMsg, unsigned int  iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
		return LEVEL_SYSTEM_ERROR;
...
int CTCPClientSocket::SendData(const char* szReqBuf,unsigned int iReqLen,char* szErrorMsg,unsigned int iMaxErrorLen)
{
	if(NULL == szErrorMsg)
	{
		return LEVEL_SYSTEM_ERROR;
...
int CTCPClientSocket::CloseSocket()
{
	if(m_iSock < 0)
	{
		m_iSock = -1;
